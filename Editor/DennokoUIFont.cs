...
namespace DennokoWorks.MatSync
{
    ...
    ///
    /// EditorWindow 側でフォントを直接生成せず、
    /// ルート要素に対して Apply を 1 回呼ぶ。
    /// </summary>
    [InitializeOnLoad]
    internal static class DennokoUIFont
    {
        ...
        private const string WarmupJapanese =
            ...
            "比較左右同時編集元先結果詳細除外理由全件クリア再終了テクスチャ含保持差分のみ未変更項目予定なしプロパティー状態同じ値存在しません異方性反射輪郭線透過描画内部派生値手動停止重複件抽出メッシュ";
        ...
        private static void Tick()
        {
            ...
            _nextTick = EditorApplication.timeSinceStartup + 2.0;
